...
본 절차의 leaf 작성 사양(산출물 구조·챕터별 절 패턴·위임 규칙·검증 항목·출처 라이프사이클)은 `references/leaf-spec.md` 가 단일 진실 소스다. 아래 단계는 그 사양 위에서 모드 동작만 기술한다.
...
5. `.claude/docs/INDEX.md` 인덱스 표에서 해당 leaf 행의 상태 열을 `review` 로 갱신. 챕터만이면 해당 챕터의 모든 leaf 행 일괄 갱신.
