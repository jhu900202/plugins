...
name: arc42
description: >
  프로젝트에 사용할 **arc42 표준 문서 구조**를 생성·관리.
  자연어 트리거: "arc42 골격 만들어줘", "1-1 챕터 작성", "1-1 확정" 등.
  슬래시 커맨드 진입점은 `commands/arc42-{create,list,update,confirm}.md` 참조.
user-invocable: true
...
본 스킬은 두 가지 경로로 진입할 수 있습니다.

**1. 슬래시 커맨드** (권장 — `/discover` UI 노출, 인자 자동완성):

| 커맨드 | 동작 |
|------|------|
| `/arc42-create` | 골격 생성 |
| `/arc42-list` | 진척 확인 |
| `/arc42-update <chapter-id>` | 챕터 본문 작성 |
| `/arc42-confirm <chapter-id>` | 챕터 검토 승인 |

각 커맨드 정의는 플러그인 루트의 `commands/arc42-*.md` 에 있으며, 본 SKILL.md 의 해당 모드 섹션을 그대로 따릅니다.

**2. 자연어 트리거** (자동 활성화):

- "arc42 골격 만들어줘" → create
- "arc42 진척 보여줘" → list
- "1-1 챕터 작성해줘" → update 1-1
- "1-1 확정" → confirm 1-1
